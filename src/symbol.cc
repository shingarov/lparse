...
void SymbolTable::SetValue(const char *key, long val, int)
{
  long index = 0;
...
long SymbolTable::CheckName(const char *key, int ar1)
{
  int len = strlen(key);
  int ar2 = 0;
  ...
  for ( long i = 0; i < MAX_SIZE; i++) {
    ...
    if (!strncmp(items[i]->symbol, key, len) && items[i]->symbol[len] == '/') {
      string whole(items[i]->symbol);
      string::size_type start = len + 1;
      string str = whole.substr(start, string::npos);
...
unsigned long FunctionTable::FindIndex(const char *key, long mask)
{
  long index = 0,ind1 = 0, ind2 = 0;
  long tries = 20;
...
long FunctionTable::Insert(const char *key)
{
  long index = 0;
  InstFunc fun = NULL;
  LibraryNode *ln = NULL;
...
long FunctionTable::Register(const char *key, InstFunc p, int val)
{
...
long FunctionTable::Lookup(const char *key)
{
  debug(DBG_SYMBOL, 2, "Looking for symbol (%s)", key);
...
InstFunc FunctionTable::GetFunction(const char *key)
{
  long index = FindIndex(key, NORMAL_MASK);
  if (!items[index] || !items[index]->valid)
    return NULL;
