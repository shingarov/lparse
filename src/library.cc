...
InstFunc LibraryNode::FindSymbol(const char *key)
{
  InstFunc fun = NULL;
...
const char *lparse_get_symbolic_constant_name(long arg)
{
  char *result = 0;
  if (!IS_CONSTANT(arg)) {
...
long lparse_get_symbolic_constant_index(const char *str)
{
  return constant_table->Lookup(str);
}

int lparse_symbol_exists(const char *str)
{
...
long lparse_create_new_symbolic_constant(const char *str)
{
